...
def make_announcement(ports, as_seq=None, origin=3450):
    ...
    for port in ports:
        if not as_seq is None:
            to_append = list(as_seq_set) + [origin]
            r = requests.post("http://localhost:{}/{}?as_seq={}".format(str(port), PREFIX, ",".join([str(x) for x in to_append])))
        else:
            r = requests.post("http://localhost:{}/{}?as_seq=".format(str(port), PREFIX))
...
def measure_single_source(frrp_run, graph, atlas_src, path=None, poison_cache=None, poisons=None, first=False):
    ...
    for current_path_asn in path:
        if current_path_asn in [int(atlas_src.asn), 3450]:
        ...
        log_message('Making announcement to {} and waiting {} seconds for AS SEQ: {}'.format(PREFIX, CONST_WAIT_TIME, current_poison_set))
        make_announcement(ports, as_seq=current_poison_set)
...
def main():
    ...
    if len(sys.argv) < 5:
        print('usage: long_path.py <EXP_ID> <ATLAS_AS_FILE> <PREFIX> <WAIT_TIME> <PORT1> <PORT2>')
        sys.exit(1)
