...
def run_measurements(producer, port):
    ...
    start_collector = producer.send('exabgp', {'SENT_TIME': int(time.time()),
                                      ...
                                      'ACTION_PAYLOAD': {'COLLECTOR_ID': 'long_path_self_append_wait40',
    ...
    # We could go to 500, but we don't even see that, so we go to 300
    with open('{}-length-sent-by5.txt'.format(port), 'w') as f:
        for i in range(5, 300, 5):
            sent_time = int(time.time())
            rib_client = NLRIClient('208.45.214.0/24', port=port, as_seq=[3450]*i)
            f.write('{},{}\n'.format(sent_time, i))
            time.sleep(2400)
